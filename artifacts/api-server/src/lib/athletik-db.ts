...
//
// The Streamlit/Python side stores passwords as:
//   pbkdf2:<salt_hex>:<sha256_hex>
// where:
//   - salt_hex is a 32-char hex string generated by secrets.token_hex(16)
//   - the PBKDF2 call is:  hashlib.pbkdf2_hmac("sha256",
//                              password.encode("utf-8"),
//                              salt.encode("utf-8"),   ← hex string used AS UTF-8 bytes
//                              260_000)
//   - iterations = 260,000 (fixed, NOT stored in the hash)
//
// The old implementation wrongly used split(":") which treated the salt string
// as an iteration count and produced a huge RangeError.
function verifyPassword(plain: string, stored: string): boolean {
  if (stored.startsWith("pbkdf2:")) {
    const rest = stored.slice("pbkdf2:".length);       // "<salt_hex>:<hash_hex>"
    const sep = rest.indexOf(":");
    if (sep === -1) return false;
    const saltHex = rest.slice(0, sep);                // the hex string itself
    const expectedHash = rest.slice(sep + 1);          // 64-char hex-encoded sha256
    if (!saltHex || !expectedHash) return false;

    try {
      const derived = pbkdf2Sync(
        plain,
        Buffer.from(saltHex, "utf-8"),   // ← UTF-8 bytes of the hex string (matches Python)
        260_000,                          // ← fixed iterations (matches Python)
        32,
        "sha256",
      ).toString("hex");
  ...
  // Legacy: plain SHA-256 hex (no salt)
  const { createHash } = require("crypto");
  const legacy = (createHash("sha256") as import("crypto").Hash).update(plain).digest("hex");
