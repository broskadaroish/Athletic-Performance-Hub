...
- In a classified club detail view, all commercial fields come exclusively from its `vereine` contract record; user fields are contact/account data only. Normalize legacy package keys with the club/technical-tenant context before looking up a package or price.
...
For a club, never fall back to a user package, status, dates, Stripe IDs, or historical customer number. Unknown package values must remain visibly unassigned rather than defaulting to a trainer plan.